 ...
 * You must write your own separate file (Name: conv_openmp.c),
 * and implement the following function inside it:
 *    void conv_openmp(int* M, int w, int* K, int k, int* C);
 * You are NOT allowed to modify any code in this template file.
 *
...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        fprintf(stderr, "Usage (OpenMP): %s MATRIX KERNEL ANSWER\n", argv[0]);
        return 1;
    }
